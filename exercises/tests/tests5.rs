...
unsafe fn modify_by_address(address: usize) {
    // SAFETY: The function's safety contract guarantees that `address` is a valid
    // pointer to a mutable u32, so converting it to a *mut u32 and dereferencing is safe.
    unsafe {
